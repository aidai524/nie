...
- **唯一的既有行为变更**：`GET /` 在这之前是 404，现在是面板本身（200）；`GET /index.html` 与 `/dashboard.js` 从 404 变为 200。其余所有路径与行为逐字不变（特别是 `/health/` 这类尾斜杠归一化仍然成立）。既有用例 `带尾斜杠的路径也能匹配` 里那句「`/` 应为 404」的断言必须随之更新。
