...
| 列 | 含义 |
|---|---|
| 币对 | 白名单里的 `源 → 目标`，下方小字是**目标链**（白名单是枢纽辐射形状，目标链是区分维度） |
| 状态 | **服务端判定的结果**，不是页面自己算的：`正常` / `偏离` / `失败` / `未报价`（尚未采集到） |
| 付 → 得 | 这一对当前的报价：付出多少源币、得到多少目标币 |
| USD | 源侧金额的美元估值 |
| 成本 | 这次报价的**总损耗**：`(付出美元 − 收到美元) / 付出美元`。含对方手续费与桥接费 |
| 较基准 | 当前报价相对**近 1 小时成功报价中位数**的变化。**与告警判定用的是同一个数** |
| 可按 | **已验证可通过的最大金额档位**，以名义美元表示（`$1M` / `$100k` / …）。`—` = 测过但全档不通；`?` = 本次没测它（拿不到价格，无法折算金额）；整列消失 = 深度扫描已关闭 |
| 延迟 | 单次询价耗时 |
| 最后报价 | 相对时间 |
